...
async function resolveFont(font: string, fontUrl?: string): Promise<string> {
  ...
  if (!effectiveUrl) {
    // A custom family was supplied without a URL â€“ make sure it is ready (in
    ...
    if (document.fonts && document.fonts.load) {
      try {
        ...
        // Ignore â€“ fall back to whatever the browser provides.
      }
    }
    return font;
  }
  try {
    ...
    if (document.fonts && document.fonts.load) {
      try {
        ...
        // Ignore â€“ we still attempt to render with the requested font.
      }
    }
    return resolved;
  } catch (error) {
...
class App {
  ...
  addEventListeners() {
    ...
    // Scope interaction to the gallery so the landing page can still scroll.
